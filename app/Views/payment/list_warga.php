    ...
    <main class="max-w-4xl mx-auto px-4 py-6">
        ...
        <div id="wargaList" class="space-y-3">
            <?php foreach($warga_list as $w): ?>
                <a href="/payment/detail/<?= $tarif['kode_tarif'] ?>/<?= $w['nikk'] ?>" 
                   class="warga-card block glass rounded-2xl p-4 hover:bg-white dark:hover:bg-slate-800/80 transition-all duration-200 border border-transparent hover:border-indigo-500/20 active:scale-[0.99] animate__animated animate__fadeInUp"
                   data-name="<?= strtolower($w['nama']) ?>"
                   data-nikk="<?= strtolower($w['nikk']) ?>">
                    
                    <div class="flex items-center justify-between">
                        <div class="flex items-center space-x-3">
                            <div class="w-10 h-10 rounded-full bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-slate-500 dark:text-slate-400 font-bold shrink-0">
                                <?= substr($w['nama'], 0, 1) ?>
                            </div>
                            <div>
                                <h4 class="font-bold text-slate-800 dark:text-white line-clamp-1"><?= $w['nama'] ?></h4>
                                <div class="flex items-center gap-2 mt-0.5">
                                    <span class="text-xs text-slate-500 dark:text-slate-400 font-mono"><?= $w['nikk'] ?></span>
                                    <span class="text-[10px] bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 px-1.5 py-0.5 rounded">RT <?= $w['rt'] ?></span>
                                </div>
                            </div>
                        </div>
