...
<body class="min-h-screen bg-slate-50 dark:bg-dark transition-colors duration-300 pb-24 md:pb-6">
    ...
    <main class="max-w-4xl mx-auto px-4 py-6">
        ...
        <!-- List Data -->
        <div id="kkGrid" class="grid grid-cols-1 md:grid-cols-2 gap-3">
            <?php foreach($dataKK as $k): ?>
                <div class="kk-card glass rounded-2xl p-4 flex items-center justify-between shadow-sm animate__animated animate__fadeInUp border border-transparent hover:border-indigo-500/20 transition-all"
                     data-name="<?= strtolower($k['kk_name']) ?>"
                     data-code="<?= strtolower($k['code_id']) ?>">
                    
                    <div class="flex items-center space-x-3 overflow-hidden flex-1">
                        <div class="w-10 h-10 rounded-xl bg-gradient-to-br from-emerald-400 to-teal-500 flex items-center justify-center text-white font-bold text-lg shrink-0 shadow-lg shadow-emerald-500/20">
                            <i class="fas fa-qrcode"></i>
                        </div>
                        <div class="overflow-hidden min-w-0">
                            <h4 class="font-bold text-slate-800 dark:text-white truncate"><?= $k['kk_name'] ?></h4>
                            <div class="flex items-center gap-1 mt-0.5">
                                <span class="text-[10px] text-slate-500 bg-slate-100 dark:bg-slate-700/50 px-1.5 py-0.5 rounded font-mono">
                                    <?= $k['code_id'] ?>
                                </span>
                                <?php if(!empty($k['nikk'])): ?>
                                <span class="text-[10px] text-indigo-500 bg-indigo-50 dark:bg-indigo-900/30 px-1.5 py-0.5 rounded font-mono">
                                    <?= $k['nikk'] ?>
                                </span>
                                <?php endif; ?>
                            </div>
                    ...
                    <div class="flex items-center space-x-1 shrink-0 ml-2">
                        <button onclick="showQR('<?= $k['code_id'] ?>', '<?= $k['kk_name'] ?>')" class="w-9 h-9 rounded-xl bg-slate-50 dark:bg-slate-800 text-slate-600 dark:text-slate-300 flex items-center justify-center hover:bg-indigo-100 hover:text-indigo-600 dark:hover:bg-indigo-900/30 transition-all">
                            <i class="fas fa-qrcode text-sm"></i>
                        </button>
                        <button onclick='openEditModal(<?= json_encode($k) ?>)' class="w-9 h-9 rounded-xl bg-slate-50 dark:bg-slate-800 text-slate-600 dark:text-slate-300 flex items-center justify-center hover:bg-amber-100 hover:text-amber-600 dark:hover:bg-amber-900/30 transition-all">
                            <i class="fas fa-edit text-sm"></i>
                        </button>
                        <button onclick="confirmDelete(<?= $k['id'] ?>, '<?= $k['kk_name'] ?>')" class="w-9 h-9 rounded-xl bg-slate-50 dark:bg-slate-800 text-slate-600 dark:text-slate-300 flex items-center justify-center hover:bg-rose-100 hover:text-rose-600 dark:hover:bg-rose-900/30 transition-all">
    ...
    <div class="md:hidden bottom-nav">
        ...
        <a href="/kk" class="nav-item active">
        ...
        document.addEventListener('DOMContentLoaded', () => {
             tSelect = new TomSelect('#nikk', {
                ...
                load: function(query, callback) {
                    ...
                    fetch('/kk/search?q=' + encodeURIComponent(query))
